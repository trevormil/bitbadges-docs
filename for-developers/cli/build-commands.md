...
All build commands accept friendly inputs -- use coin symbols like `USDC.noble` instead of raw IBC denominations, and duration shorthands like `30d` instead of millisecond timestamps. The output is a fully-formed transaction message that can be signed and broadcast using the SDK, the BitBadges frontend, or the chain binary.

> **Which USDC?** Examples on this page spell the payment coin `USDC.noble`
> on purpose. From `bitbadges@0.43.0` the bare symbol `USDC` resolves to the
> canonical Injective-routed denom, which has no circulating supply yet — a
> collection priced in it cannot be settled by anyone. Use `USDC.noble` (or
> `ubadge`) until the canonical route is bridged. See
...
```bash
bb build vault --backing-coin USDC.noble \
  --name "My Vault" \
  --symbol vUSDC \
...
bb build subscription --interval monthly \
  --price 10 --denom USDC.noble --recipient bb1... \
...
```bash
bb build bounty --amount 500 --denom USDC.noble \
...
bb build product-catalog \
  --products '[{"name":"Widget","price":25,"denom":"USDC.noble","maxSupply":100}]' \
  --store-address bb1...
```
...
bb build prediction-market --verifier bb1... \
  --denom USDC.noble --name "Will X happen by 2027?"
```

| Flag | Required | Description |
|------|----------|-------------|
| `--verifier <address>` | Yes | Market resolver address |
| `--denom <symbol>` | No | Payment coin (default: USDC.noble) |
...
```bash
bb build smart-account --backing-coin USDC.noble \
...
```bash
bb build credit-token --payment-denom USDC.noble \
...
bb build intent --address bb1... \
  --collection-id 5 \
  --pay-denom USDC.noble --pay-amount 100 \
...
bb build listing --address bb1... \
  ...
  --price 50 --denom USDC.noble --max-sales 1 --expiration 30d
```
...
bb build bid --address bb1... \
  --collection-id 7 --token-ids 4 \
  --price 40 --denom USDC.noble --expiration 7d

# Collection-wide bid (any token in the collection)
bb build bid --address bb1... \
  --collection-id 7 \
  --price 40 --denom USDC.noble
```
...
bb build pm-sell-intent --address bb1... \
  ...
  --amount 10 --price 50 --denom USDC.noble
```
...
```bash
# Inline JSON
bb build vault --json '{"backingCoin":"USDC.noble","name":"My Vault"}'

# From a file
bb build vault --json ./params.json

# From stdin
echo '{"backingCoin":"USDC.noble"}' | bb build vault --json -
```
