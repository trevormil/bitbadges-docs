...
You can use the attempt ID to verify claim success on your end via the API.
...
* No User Inputs - The plugin should be able to function at any time without any custom user inputs.
...
As a design decision, we do NOT want to handle your authentication or sensitive values. Treat BitBadges as a middleman. Authentication should be fully managed by you from start to finish. This applies to both the creator and the end claiming user.
