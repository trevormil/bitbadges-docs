...
A flexible storage system for maintaining lists of:
...
These can be thought of as a separate store for maintaining a list of users. You then "attach" stores to their respective plugins when creating a claim.

This is an alternative to custom plugins because the validation and storage is performed by BitBAdges. You just update the store as you go with API calls or hooks.
