...
> **Building from your own code (Node/TypeScript)?** Use the [Programmatic Agent](programmatic-agent.md) — same tools + system prompt + validation loop, BYO Anthropic key, no MCP client required. One-liner: `new BitBadgesBuilderAgent({ anthropicKey }).build('…')`.
