...
By default, we expect the collection and badge metadata as shown below for the BitBadges indexer / API. Also, if the badge metadata URI includes "{id}" anywhere in the URI, it will be dynamically replaced by the corresponding badge ID number. For example: `"...x.com/metadata/{id}"` (see [Compatibility](../bitbadges-api/compatibility.md) for more info).
