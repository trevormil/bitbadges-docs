...
// Precompile address
address constant TOKENIZATION_PRECOMPILE = 0x0000000000000000000000000000000000001001;
```
...
import "./ITokenizationPrecompile.sol";

contract TokenizationContract {
    ITokenizationPrecompile public constant precompile = ITokenizationPrecompile(TOKENIZATION_PRECOMPILE);
    ...
    function getFullRange() internal pure returns (ITokenizationPrecompile.UintRange[] memory) {
        ITokenizationPrecompile.UintRange[] memory ranges = new ITokenizationPrecompile.UintRange[](1);
        ranges[0] = ITokenizationPrecompile.UintRange({
            start: 0,
...
function transfer(address to, uint256 amount) external returns (bool) {
    ...
    ITokenizationPrecompile.UintRange[] memory tokenIds = getFullRange();
    ITokenizationPrecompile.UintRange[] memory ownershipTimes = getFullRange();
...
function transferSpecificTokens(
    ...
    ITokenizationPrecompile.UintRange[] memory tokenIds = new ITokenizationPrecompile.UintRange[](1);
    tokenIds[0] = ITokenizationPrecompile.UintRange({
        start: tokenIdStart,
        end: tokenIdEnd
    });
...
function transferMultipleRanges(
    ...
    ITokenizationPrecompile.UintRange[] memory tokenIds = new ITokenizationPrecompile.UintRange[](tokenIdStarts.length);
    for (uint256 i = 0; i < tokenIdStarts.length; i++) {
        require(tokenIdStarts[i] <= tokenIdEnds[i], "Invalid range");
        tokenIds[i] = ITokenizationPrecompile.UintRange({
...
function allowIncomingFromAll() external returns (bool) {
    ITokenizationPrecompile.UserIncomingApproval memory approval = ITokenizationPrecompile.UserIncomingApproval({
        approvalId: "incoming_all",
        fromListId: "All",
        initiatedByListId: "All",
        transferTimes: new ITokenizationPrecompile.UintRange[](0), // Empty = all times
        tokenIds: new ITokenizationPrecompile.UintRange[](0), // Empty = all token IDs
        ownershipTimes: new ITokenizationPrecompile.UintRange[](0), // Empty = all ownership times
        uri: "",
        customData: ""
    });
...
function allowOutgoingTo(address to) external returns (bool) {
    ...
    ITokenizationPrecompile.UserOutgoingApproval memory approval = ITokenizationPrecompile.UserOutgoingApproval({
        approvalId: "outgoing_all",
        toListId: "All",
        initiatedByListId: "All",
        transferTimes: new ITokenizationPrecompile.UintRange[](0),
        tokenIds: new ITokenizationPrecompile.UintRange[](0),
        ownershipTimes: new ITokenizationPrecompile.UintRange[](0),
        uri: "",
        customData: ""
    });
...
function setRestrictedApproval(
    ...
    ITokenizationPrecompile.UintRange[] memory ownershipTimes = new ITokenizationPrecompile.UintRange[](1);
    ownershipTimes[0] = ITokenizationPrecompile.UintRange({
        start: timeStart,
        end: timeEnd
    });
...
contract OptimizedBadges {
    ...
    ITokenizationPrecompile.UintRange[] private fullTokenRange;
    ITokenizationPrecompile.UintRange[] private fullOwnershipRange;
    
    constructor(uint256 _collectionId) {
        collectionId = _collectionId;
        
        // Initialize cached ranges
        fullTokenRange = new ITokenizationPrecompile.UintRange[](1);
        fullTokenRange[0] = ITokenizationPrecompile.UintRange({
            start: 0,
            end: type(uint256).max
        });
        
        fullOwnershipRange = new ITokenizationPrecompile.UintRange[](1);
        fullOwnershipRange[0] = ITokenizationPrecompile.UintRange({
            start: 0,
            end: type(uint256).max
        });
    }
...
) internal pure returns (ITokenizationPrecompile.UintRange[] memory) {
    require(starts.length == ends.length, "Mismatched arrays");
    
    ITokenizationPrecompile.UintRange[] memory ranges = new ITokenizationPrecompile.UintRange[](starts.length);
    for (uint256 i = 0; i < starts.length; i++) {
        require(starts[i] <= ends[i], "Invalid range");
        ranges[i] = ITokenizationPrecompile.UintRange({
            start: starts[i],
            end: ends[i]
        });
    }
