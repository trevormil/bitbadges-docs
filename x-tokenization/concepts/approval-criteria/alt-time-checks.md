...
interface AltTimeChecks {
    ...
    /** ISO 8601 weeks of the year (1-52) when transfers should be denied. */
    offlineWeeksOfYear?: UintRange[];
    /** Timezone offset magnitude in minutes from UTC. Combined with timezoneOffsetNegative to determine direction. */
...
The `offlineWeeksOfYear` field specifies ranges of ISO 8601 weeks (1-52) when transfers should be denied. ISO 8601 defines week 1 as the week containing the first Thursday of the year. Most years have 52 weeks; some have 53.
...
-   Week-of-year values must be in the range **1-52** (inclusive), following ISO 8601
-   `timezoneOffsetMinutes` must be a non-negative integer (magnitude only; use `timezoneOffsetNegative` for direction)
